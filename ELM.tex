...
A Boolean function of $n$-variable is a map from $\F_2^n$ to $\F_2$ and $\BN$ be the set of all $n$-variable Boolean functions. The $2^n$-length binary sequence $(f(v_0), f(v_1), \ldots, f(v_{2^n-1}))$ is called as the truth table of the Boolean function $f$ corresponds to the ordered vectors of $\F_2^n$ as $v_0 = (0,0,\ldots,0), v_1= (0,0,\ldots,1),\ldots, v_{2^n-1}=(1,1,\ldots,1)$. 
...
$f(x) = \sum\limits_{u \in \F_2^n}c_u x^{a_1}x^{u_2}\cdots x^{a_n},$
...
\[\NLk{k}(f) = \frac{|\Ekn{n}{k}|}{2}- \frac{1}{2}\max_{a \in \F_2^n} |\wtk{f}{k}(a)| \textit{ and } \]
...
For a integers $n > 0$, $0 \leq k \leq n$ and fixed $a \in \F_2^n$ such that $\w(a) = l$, the following relations holds.
\begin{enumerate}
...
%Let denote $\orbs$ be the set of all orbits due the action of $\psi$ on $\F_2^n$, $\orbs_o$ be the set of orbits of odd cardinality (i.e., here $1$) and $\orbs_e$ be the set of orbits of even cardinality.
...
Let $a = (a_1, a_2, \ldots, a_n) \in \F_2^n$. Let $ a \lll \gamma$ be denoted as the cyclic left shift of $a$ by $ \gamma$ bits and $ a ^\wedge b$ be denoted the vector bitwise XOR operation. Then $$ \w(a ^\wedge (a \lll 1) ) = 2 ( \w(a) - |\{ (a_i, a_{i (\mod(n)) + 1}) : a_i + a_{i (\mod(n) + 1} = 0 \text{ for } i \in [1,n] \}|). $$
\end{lemma}
\begin{proof}
Let $a = (a_1, a_2, \ldots, a_n) \in \F_2^n$. Let $ \overleftarrow{a} = a \lll 1 = (a_2, a_3, \ldots, a_n,a_1)$. Now $ \w(a ^\wedge \overleftarrow{a}) = \w( a_1 + a_2, a_2 + a_3, \ldots , a_{n-1} + a_n , a_n + a_1) = a_1 + a_2 + a_2 + a_3 + \cdots + a_{n-1} + a_n + a_n + a_1 - 2 \times  |\{ (a_i, a_{i (mod(n) + 1}) : a_i + a_{i (mod(n) + 1} = 0 \text{ for } i \in [1,n] \}| $, since each consecutive pair $(a_i, a_{i (mod(n) + 1})$ such that $a_i = a_{i (mod(n) + 1} = 1  $ donate 2- $1$'s to the sum $ a_1 + a_2 + a_2 + a_3 + \cdots + a_{n-1} + a_n + a_n + a_1 $ but  $a_i + a_{i (mod(n) + 1} = 0$.
\end{proof}
...
= \left \{ \begin{array}{ll}
 \sum_{\substack{i=0 \\ i:odd}}^k {d \choose i} {(n-d) \choose (k-i)}   &  \text{ if } c \in \F_2^n \setminus \orbs_o  \\
0              &  \text{ if } c \in \orbs_o.
\end{array} \right.$$
...
\end{lemma} 
\begin{proof}
Let $x = (x_1,x_2, \ldots,x_n) \in \Ekn{k}{n} \subset \F_2^n $ and $ c = (c_1,c_2,\ldots,c_n) \in \F_2^n$.
\begin{eqnarray}
c \cdot (x+\psi(x)) & = & c_1(x_1 + x_{n_1}) + c_2 (x_2 + x_1) + \cdots + c_{n_1}( x_{n_1} + x_{{n_1}-1}) \nonumber \\
&& +c_{{n_1}+1}(x_{{n_1}+1}+ x_{{n_1}+ {n_2}}) + \cdots + c_{n_1+n_2}(x_{n_1+n_2} + x_{n_1+n_2- 1})\nonumber\\
&& + \cdots \nonumber\\
...
Then $$\NLk{k}(f) ~\geq ~ 
\left\{ \begin{array}{ll}
       \frac{|\Ekn{k}{n}|}{2} - \frac{1}{2}\max_{a\in \F_2^n}|\sum_{\substack{i=0 \\ i:odd}}^k {d \choose i} {(n-d) \choose (k-i)}| & \text{ if }  k \not\preceq n \\
        \frac{|\Ekn{k}{n}|}{2} - \frac{1}{2}\max_{a\in \F_2^n}|\sum_{\substack{i=0 \\ i:odd}}^k {d \choose i} {(n-d) \choose (k-i)} + 1 | & \text{ if } k \preceq n
    \end{array}   \right.$$
\end{theorem}
\begin{proof}
...
\left| \sum_{\orb \in \orbs_{e,k}} \sum_{x \in \orb} (-1)^{ f(x) + a \cdot x} \right| 
    ...
    & \leq & \left\{
    \begin{array}{ll}
     \sum_{\substack{i=0 \\ i:odd}}^k {d \choose i} {(n-d) \choose (k-i)}   & \text{ if } a \in \F_2^n \setminus \orbs_o \\
...
|\wtk{f}{k}(a)| \leq \left\{ \begin{array}{ll}
        \sum_{\substack{i=0 \\ i:odd}}^k {d \choose i} {(n-d) \choose (k-i)} + 1 & \text{ if } a \in \F_2^n \setminus \orbs_o \\
        1      & \text{ if } a \in \orbs_o
    \end{array}   \right. 
\end{equation}
Hence, the nonlinearity of $f_\psi$ satisfies
\begin{eqnarray*}
\NLk{k}(f_\psi) & ~\ge~ & 2^{n-1}-\frac{1}{2} \max_{a\in \F_2^n}| \wtk{f}{k}(a)|
~\geq ~ 
\left\{ \begin{array}{ll}
       \frac{|\Ekn{k}{n}|}{2} - \frac{1}{2}\max_{a\in \F_2^n}|\sum_{\substack{i=0 \\ i:odd}}^k {d \choose i} {(n-d) \choose (k-i)}| & \text{ if }  k \not\preceq n \\
        \frac{|\Ekn{k}{n}|}{2} - \frac{1}{2}\max_{a\in \F_2^n}|\sum_{\substack{i=0 \\ i:odd}}^k {d \choose i} {(n-d) \choose (k-i)} + 1 | & \text{ if } k \preceq n
...
\begin{lemma}
Let $a = (a_1, a_2, \ldots, a_n) \in \F_2^n$. Let $ \rho(a)$ be denoted as the cyclic left shift of $a$ by $ 1 $ bits and $ a + b$ be denoted the vector bitwise XOR operation. Then $$ \w(a + \rho(a) ) = 2 ( \w(a) - |\{ (a_i, a_{i (\mod(n)) + 1}) : a_i + a_{i (\mod(n) + 1} = 0 \text{ for } i \in [1,n] \}|). $$
\end{lemma}
\fi
...
%Consider $c+\psi^{-1}(c) = (\overline c_1, \overline c_2, \ldots, \overline c_n) \in \F_2^n$ where $\overline c_1 = c_1 + c_2, \overline c_2 = c_2 + c_3, \ldots, \overline c_n = c_n + c_{n-n_w+1}$.
...
= \frac{1}{2} (|\Ekn{n}{k}| - \kraw{k}{l}{n}).$$
\iffalse
If $ c \in \F_2^n \setminus \orbs_o$, then we have
\begin{align*}
...
$$\left| \{x \in \Ekn{k}{n} : c \cdot (x + \psi(x))= 1\} \right|
= \left \{ \begin{array}{ll}
\frac{1}{2} \left[ |\Ekn{k}{n}| - \kraw{k}{\w(c + \psi(c))}{n} \right]  &  \text{ if } c \in \F_2^n \setminus \orbs_o  \\
0              &  \text{ if } c \in \orbs_o.
\end{array} \right. $$ \fi
\end{proof}
