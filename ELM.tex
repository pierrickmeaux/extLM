...
& = & \frac{1}{2} \left[ \sum_{x \in \Ekn{k}{n} \setminus \orbs_o} (-1)^{ f(x)} \left((-1)^{a \cdot x} - (-1)^{ a \cdot \psi(x)}\right) \right]
\end{eqnarray*}
...
\begin{eqnarray*}
\sum_{\orb \in \orbs_{e,k}} \sum_{x \in \orb} (-1)^{ f(x) + a \cdot x} 
& = & \frac{1}{2} \left[ \sum_{\substack{x \in \Ekn{k}{n} \setminus \orbs_o \\ a \cdot (x + \psi(x))= 1}} (-1)^{f(x)} ((-1)^{ a \cdot x} - (-1)^{a \cdot \psi(x)})  \right] 
=  \sum_{\substack{x \in \Ekn{k}{n} \setminus \orbs_o \\ a \cdot (x + \psi(x))= 1}} (-1)^{f(x)+ a \cdot x}. 
...
|\wtk{f}{k}(a)| & \leq & \left\{\begin{array}{ll}
    |\sum\limits_{\substack{x \in \Ekn{k}{n} \setminus \orbs_o \\ a \cdot (x + \psi(x))= 1}} (-1)^{f(x)+ a \cdot x}| & \text{if } k \not\preceq n\\
    |\sum\limits_{\substack{x \in \Ekn{k}{n} \setminus \orbs_o \\ a \cdot (x + \psi(x))= 1}} (-1)^{f(x)+ a \cdot x}| + 1 & \text{if } k \preceq n.
    \end{array}\right. \nonumber\\
    & = & \left\{\begin{array}{ll}
...
Hence, the nonlinearity of $f_\psi$ satisfies
\begin{eqnarray*}
\NLk{k}(f_\psi) ~ = ~ \frac{|\Ekn{k}{n}|}{2} -\frac{1}{2} \max_{a\in \F_2^n}| \wtk{f}{k}(a)|
...
\pmnote{Improvement, under construction

We simplify the bound on $\NLk{k}(f_\psi)$ by bounding the minimum $\min_{0 \leq l \leq n} \kraw{k}{l}{n}$ and discarding some values of $l$. 

Since $l = \w(a+\psi^{-1}(a))$, $a$ and $\psi^{-1}(a)$ have the same Hamming weight, hence $l$ is even. 
We want to discard the cases $l=0$ and $l=n$.
