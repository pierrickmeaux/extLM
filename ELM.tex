...
Let $\FF_2^n$ be the vector space of dimension $n$ over the binary field $\FF_2$. For any two vectors $a=(a_1, a_2, \ldots, a_n)$ and $b = (b_1, b_2, \ldots, b_n)$ in $\FF_2^n$, the dot product is defined as $a\cdot b = a_1 b_1 + a_2 b_2 + \cdots + a_nb_n \bmod 2$.
%and additon $+$ is by .......

A Boolean function of $n$-variable is a map from $\FF_2^n$ to $\FF_2$ and $\BN$ be the set of all $n$-variable Boolean functions. The $2^n$-length binary sequence $(f(v_0), f(v_1), \ldots, f(v_{2^n-1}))$ is called as the truth table of the Boolean function $f$ corresponds to the ordered vectors of $\FF_2^n$ as $v_0 = (0,0,\ldots,0), v_1= (0,0,\ldots,1),\ldots, v_{2^n-1}=(1,1,\ldots,1)$. 
The Hamming weight of a vector $x \in \FF_2^n$, denoted by $\w(x)$, is the number nonzero cordinates (i.e., $1$s) in the vector $x$. The support of $f \in \BN$, denoted by $\supp(f)= \{x \in \FF_2^n : f(x) = 1\}$. Therefore, the Hamming weight of the Boolean function $f$, is cardinality of $\supp(f)$ and is denoted by $wt(f)$. The function $f$ is called balanced if $\w(f)= 2^{n-1}$. Let $f(x), g(x) \in \BN$, then 
The Hamming distance between two Boolean functions $f, g \in \BN$ is defined by $\hd(f,g) = |\{x \in \FF_2^n : f(x) \neq g(x) \}|$  i.e., $\hd(f,g) = \w(f + g)$.

An $n$-variable Boolean function $f$ can be expressed as a polynomial in the ring $\FF_2[x_1,x_2,\ldots,x_n]/<x_1^2+x_1,x_2^2+x_2,\ldots,x_n^2+x_n>$, i.e. 
$f(x) = \sum\limits_{u \in \FF_2^n}c_u x^{a_1}x^{u_2}\cdots x^{a_n},$
where $c_u$ are the coefficients with a value in $\FF_2$.
...
A function $f$ is called as affine function if $f(x)= a\cdot x + b$ for $a \in \FF_2^n$ and $b \in \FF_2$. If $b=0$, then $f$ is also called a linear Boolean function. $\Af$ denotes the set of all $n$-variable affine functions. 
...
The Walsh-Hadamard transform of a function on $\FF_2^n$ is the map $W_f : \FF_2^n \to \R$, defined by $$\wt{f}(w)= \sum_{x \in \FF_2^n} (-1)^{f(x)+ w \cdot x}.$$   
...
It can be verified that $\NL(f) = 2^{n-1} - \frac{1}{2}\max\limits_{w \in \FF_2^n} |\wt{f}(w)|$.

We denote $\Ekn{n}{k} = \{x \in \FF_2^n : \w(x)=k\}$ and $\wkn{n}{k}(f) = |\{x \in \Ekn{n}{k} : f(x) = 1 \}| = |\supp(f) \cap \Ekn{n}{k}|$. Accordingly, the Hamming distance of two functions $f, g \in \BN$ on $\Ekn{k}{n}$ denoted as $\mdist{n}{k}(f,g) = |\{x \in \Ekn{n}{k} : f(x) \neq g(x)\}|$. The cryptographic criteria like balancedness, nonlinearity and algebraic immunity of a function $f$ defined over $\FF_2^n$ can also be defined, if we restrict $f$ to the set $\Ekn{k}{n}$. For two integers $m, n$ with $m \leq n$, we define $[m,n] = \{m, m+1, \ldots, n\}$.

\noindent\textbf{\definitionname(Weightwise Almost Perfectly Balanced (WAPB)}
A Boolean function $f \in \BN$ is said to be weightwise almost perfectly balanced (WAPB) if for all $k \in [0,n]$, \[\wkn{n}{k}(f)=
\begin{cases}
...
\end{cases} \]
To make a WPB Boolean function balanced on $\FF_2^n$, we impose $f(0,0,\ldots,0) \neq f(1,1,\ldots,1)$. Using Lucas' Theorem~\cite{LucasTh}, we have that a WPB function exists only if, $n$ is a power of $2$. Hence, there are $\displaystyle {2\prod_{k = 1}^{n-1} {{{n \choose k}} \choose {{n \choose k}/2}}}$ balanced WPB Boolean functions.
...
The nonlinearity of $f \in \BN$ over $\Ekn{n}{k}$, denoted as $\NLk{k}(f)$, is the Hamming distance of $f$ to the set of all affine functions $\Af$ when evaluated over $\Ekn{n}{k}$. That is, $\NLk{k}(f) = \min_{g \in \Af} \mdist{n}{k}(f,g) = \min_{g \in \Af} \wkn{n}{k}(f+g)$.\\
...
\begin{lemma}~\cite{TOSC:CarMeaRot17}\label{lem:wnl}
If $f \in \BN$ then for $k \in [0,n]$, 
\[\NLk{k}(f) = \frac{|\Ekn{n}{k}|}{2}- \frac{1}{2}\max_{a \in \FF_2^n} |\wtk{f}{k}(a)| \textit{ and } \]
\[\NLk{k}(f) \leq \frac{1}{2} [|\Ekn{n}{k}| - \sqrt{|\Ekn{n}{k}|}] = \frac{1}{2} [{n \choose k} - \sqrt{{n \choose k}}].\]
\end{lemma}
...
$$ K_k(x,n)= \sum_{j=0}^{k} {x \choose j} {n-x \choose k-j} \text{ for } k = 0, 1, \ldots n.$$
...
For a integers $n > 0$, $0 \leq k \leq n$ and fixed $a \in \FF_2^n$ such that $\w(a) = l$, the following relations holds.
\begin{enumerate}
\item $\sum\limits_{x \in \Ekn{k}{n}} (-1)^{a.x} = \kraw{k}{l}{n}$.
\item If $l_{a,b}(x)= a \cdot x + b$, where $a \in \FF_2^n, b \in \FF_2$, be an affine Boolean function then 
$$\wkn{n}{k}(l_{a,b}) = \frac{1}{2} (|\Ekn{k}{n}| - (-1)^b \kraw{k}{l}{n}).$$
\end{enumerate}
\end{theorem}
\iffalse
Let $l_{a,b}$ be an $n$-variable affine Boolean function defined as $l_{a,b}(x)= a \cdot x + b$ for $a \in \FF_2^n$ and $ b \in \FF_2^n$, then using the Theorem~\ref{thm:Kraw}, the Hamming weight of $f$ in $\Ekn{k}{n}$ is given as:
\begin{align*}
...
A Boolean function $f$ is rotation symmetric (RotS) if and only if for any $(x_1,x_2,\ldots,x_n)\in \FF_2^n$,
...
Therefore, 2-RotS Boolean functions have the alternative truth value for the lexicograpphically ordered vectors in every orbit obtained by the action of permutation group $P$ on $\FF_2^n$. 
...
\begin{constr}\label{cons:GA}
\caption{Construction of 2-RotS WAPB Boolean function}
\begin{algorithmic}[1]
% \Require $ v=(0,0,\ldots,0) \in \FF_2^{n+1}$
...
Here, $ord(\psi) = 2^{a_w} = n_w$. Hence, the cardinality of orbits obtained due the action of $G$ on $\FF_2^n$ are of power of $2$ i.e., $|O_\psi(x)| = 2^l$ where $0 \leq l \leq a_w$ for $x \in \FF_2^n$.
...
\end{lemma}
\begin{proof}
For a vector $x \in \FF_2^n$ is having an orbit of cardinality $1$ i.e., $|O_\psi(x)| = 1$ if and only if the cordinates of $x$ present in the cycles are of same value i.e., 
...
\end{align}
As each partition of cordinates can be either $0$ or $1$, there are $2^w$ vectors $x$ in $\FF_2^n$ satisfying Equation~\ref{eqn:part} and hence $|O_\psi(x)| = 1$.
\end{proof}

Since every orbit contains the vectors of same weight, we denote the weight of an orbit is the weight of vectors in the orbit i.e, $\w(O_\psi(x)) = \w(x)$ for $x \in \FF_2^n$. Further, for $x = (x_1, x_2,\ldots, x_n), y = (y_1, y_2, \ldots, y_n) \in \FF_2^n$, we say $y$ covers $x$ (i.e., $x \preceq y $), if $x_{i} \leq y_{i}, \forall 1 \leq i \leq n$ i.e., $y_i = 1$ if $x_i = 1, \forall 1 \leq i \leq n$.
...
For a Boolean function $f_\psi \in \BN$, if $f_\psi(\psi(x)) = 1+f_\psi(x)$ holds for all $x \in \FF_2^n \setminus \orbs_o$ where $\orbs_o$ is the set of vectors whose orbit cardinality is $1$, then $f_\psi$ is WAPB.
\end{proposition}
...
\State For every orbit $\orb$ in $\FF_2^n$ due to the action of $G = \langle \psi \rangle$, do the following:
\If{$|\orb|$ is even}
...
\begin{theorem}
The number of orbits generated due the action of $\psi$ on $\FF_2^n$ is
...
Now to apply Burnside's lemma, for every $k \in \{1, 2, \ldots, n_w\}$, we need to compute the number of fixed vectors $z \in \FF_2^n$ by $\psi_n^k$ i.e., $\psi_n^k(z) = z$. That is, for every $k \in \{1, 2, \ldots, n_w\}$, we need to compute the number of vectors $z \in \FF_2^n$ such that $\rho_{n_1}^k(z_1) = z_1, \rho_{n_2}^k(z_2) = z_2, \ldots, \rho_{n_w}^k(z_w) = z_w$ where $z = (z_1,z_2, \ldots, z_w)$ and $z_1 \in \FF_2^{n_1}, z_2 \in \FF_2^{n_2}, \ldots, z_w \in \FF_2^{n_w}$.
...
$\rho_{n_i}^k$ fixes $2^{\gcd(n_i,k)}$ number of $z_i \in \FF_2^{n_i}$. Therefore, $\psi^k$ fixes $2^{\gcd(n_1,k)+ \gcd(n_2,k) + \cdots + \gcd(n_w,k)}$ number of $z \in \FF_2^n$. Hence, by using the Burnside Lemma, the number of orbits is\\
$\displaystyle{g_n = \frac{1}{n_w} \sum_{\pi \in G} |fix_{\FF_2^n}(\pi)| 
...
$$\left| \{x \in \FF_2^n \setminus \orbs_o : c \cdot (x + \psi(x))= 1\} \right| 
= \left \{ \begin{array}{ll}
2^{n-1}  &  \text{ if } c \in \FF_2^n \setminus \orbs_o  \\
0              &  \text{ if } c \in \orbs_o.
\end{array} \right.$$
\end{proposition}

\begin{proof}
Now for any $x = (x_1, x_2, \ldots, x_n), c = (c_1, c_2, \ldots, c_n) \in \FF_2^n$,
\begin{eqnarray}
...
\left| \{x \in \FF_2^n : c \cdot (x + \psi(x))= 1\} \right| = \w(c \cdot (x+\psi(x))) 
= \left\{ \begin{array}{ll}
2^{n-1} & \text{if } c \in \FF_2^n \setminus \orbs_o \\
0 & \text{if } c \in \orbs_o
\end{array} \right.
...
\end{theorem}
\begin{proof}
Let $a \in \FF_2^n$ and $\psi \in \Sn$ be the permutation defined as in Equation~\ref{eqn:cycperm}.
As $\w(n) = w$, from Lemma~\ref{lem:orbNo} there are $2^w$ orbits with cardinality $1$ and remaining orbits are of even cardinality. 
%Let denote $\orbs$ be the set of all orbits due the action of $\psi$ on $\FF_2^n$, $\orbs_o$ be the set of orbits of odd cardinality (i.e., here $1$) and $\orbs_e$ be the set of orbits of even cardinality.
Then the Walsh spectrum of $f_\psi$ at $a$ is as follows.
\begin{eqnarray} \label{eqn:ws}
\wt{f}(a) & =  &\sum_{x \in \FF_2^n} (-1)^{f(x)+a \cdot x} 
...
& = & \sum_{x \in \FF_2^n\setminus \orb_o : a \cdot (x + \psi(x))=1} (-1)^{f(x)+ a \cdot x}.
\end{eqnarray*}
...
\left| \sum_{\orb \in \orbs_e} \sum_{x \in \orb} (-1)^{ f(x) + a \cdot x} \right| 
    & = & \left|\sum_{x \in \FF_2^n\setminus \orb_o : a \cdot (x + \psi(x))=1} (-1)^{f(x)+ a \cdot x}\right| \nonumber\\
    & \leq & \left\{
    \begin{array}{ll}
        2^{n-1} & \text{ if } a \in \FF_2^n \setminus \orbs_o \\
...
|\wt{f}(a)| \leq \left\{ \begin{array}{ll}
        2^{n-1} + 2^w & \text{ if } a \in \FF_2^n \setminus \orbs_o \\
        2^w        & \text{ if } a \in \orbs_o
    \end{array}   \right. 
\end{equation}
Hence, the nonlinearity of $f_\psi$ satisfies
\begin{eqnarray*}
\NL(f_\psi) & ~\ge~ & 2^{n-1}-\frac{1}{2} \max_{a\in \FF_2^n}| \wt{f}(a)|
~\geq~ 2^{n-1}-\frac{1}{2} \max_{a\in \FF_2^n} \{2^{n-1}+2^w,2^w \} ~=~ 2^{n-1} - 2^{n-2} - 2^{w-1} \\
\implies \NL(f_\psi) & \ge & 2^{n-2} - 2^{w-1}.
\end{eqnarray*}
\end{proof}
In the following table we have presented the maximum nonlinearity among all $f_\psi$ for the number of variables $n = \{4, 5, \ldots, 8\}$
\begin{center}
...
Max Nonlinearity & $4$ & $12$ & $26$ & $48$ & $116$ & $236$ & $440$\\ \hline
\end{tabular}
\end{center}
...
\iffalse
-------------************------------*****************----------------*******************----------******
\begin{lemma}\label{lem:BitXOR}
Let $a = (a_1, a_2, \ldots, a_n) \in \FF_2^n$. Let $ a \lll \gamma$ be denoted as the cyclic left shift of $a$ by $ \gamma$ bits and $ a ^\wedge b$ be denoted the vector bitwise XOR operation. Then $$ \w(a ^\wedge (a \lll 1) ) = 2 ( \w(a) - |\{ (a_i, a_{i (\mod(n)) + 1}) : a_i + a_{i (\mod(n) + 1} = 0 \text{ for } i \in [1,n] \}|). $$
\end{lemma}
\begin{proof}
Let $a = (a_1, a_2, \ldots, a_n) \in \FF_2^n$. Let $ \overleftarrow{a} = a \lll 1 = (a_2, a_3, \ldots, a_n,a_1)$. Now $ \w(a ^\wedge \overleftarrow{a}) = \w( a_1 + a_2, a_2 + a_3, \ldots , a_{n-1} + a_n , a_n + a_1) = a_1 + a_2 + a_2 + a_3 + \cdots + a_{n-1} + a_n + a_n + a_1 - 2 \times  |\{ (a_i, a_{i (mod(n) + 1}) : a_i + a_{i (mod(n) + 1} = 0 \text{ for } i \in [1,n] \}| $, since each consecutive pair $(a_i, a_{i (mod(n) + 1})$ such that $a_i = a_{i (mod(n) + 1} = 1  $ donate 2- $1$'s to the sum $ a_1 + a_2 + a_2 + a_3 + \cdots + a_{n-1} + a_n + a_n + a_1 $ but  $a_i + a_{i (mod(n) + 1} = 0$.
\end{proof}

\begin{lemma} \label{lem:setSize_k}
For $n \geq 2$ as in Equation~\ref{eqn:binn}, let $\psi \in \Sn$ be the permutation as defined in Equation~\ref{eqn:cycperm}.Then 
$$\left| \{x \in \Ekn{k}{n} : c \cdot (x + \psi(x))= 1\} \right| 
= \left \{ \begin{array}{ll}
 \sum_{\substack{i=0 \\ i:odd}}^k {d \choose i} {(n-d) \choose (k-i)}   &  \text{ if } c \in \FF_2^n \setminus \orbs_o  \\
0              &  \text{ if } c \in \orbs_o.
\end{array} \right.$$
where $ d = \w( c_1 + c_2 , c_2 + c_3, \ldots, c_{n_1}+ c_1, c_{n_1 + 1} + c_{n_1 + 2} , c_{n_1 + 2} + c_{n_1 + 3}, \ldots, c_{n_1 + n_2}+ c_{n_1 + 1}, \ldots, c_{n -{n_w} + 1} + c_{n -{n_w} + 2}, c_{n -{n_w} + 2} + c_{n -{n_w} + 3}, \ldots, c_{n} + c_{n -{n_w} + 1}) $.
\end{lemma} 
\begin{proof}
Let $x = (x_1,x_2, \ldots,x_n) \in \Ekn{k}{n} \subset \FF_2^n $ and $ c = (c_1,c_2,\ldots,c_n) \in \FF_2^n$.
\begin{eqnarray}
c \cdot (x+\psi(x)) & = & c_1(x_1 + x_{n_1}) + c_2 (x_2 + x_1) + \cdots + c_{n_1}( x_{n_1} + x_{{n_1}-1}) \nonumber \\
&& +c_{{n_1}+1}(x_{{n_1}+1}+ x_{{n_1}+ {n_2}}) + \cdots + c_{n_1+n_2}(x_{n_1+n_2} + x_{n_1+n_2- 1})\nonumber\\
&& + \cdots \nonumber\\
&& + c_{n-n_w+1} (x_{n-n_w+1} + x_n) + \cdots + a_n(x_n + x_{n-1}) \label{eqn:cxk1}\nonumber\\
\implies c \cdot (x+\psi(x)) & = & x_1(c_1 + c_2) + x_2(c_2 + c_3) + \cdots + x_{n_1}(c_{n_1} + c_1)\nonumber \\
&& + x_{n_1+1}(c_{n_1+1} + c_{n_1+2}) + \cdots + x_{n_1+n_2} (c_{n_1 + n_2} + c_{n_1+1})\nonumber \\
&& + \cdots \nonumber\\
&& + x_{n-n_w+1} (c_{n-n_w+1} + c_{n-n_w+2})+ \cdots +  x_n(c_n + c_{n-n_w+1}).\label{eqn:cxk2}
\end{eqnarray}
 Here $c \cdot (x+\psi(x))$ is the zero Boolean function if and only if
$c_1 = c_2 = \ldots = c_{n_1}$; $c_{n_1+1} = c_{n_1 + 2} = \cdots = c_{n_1 + n_2}; \ldots; c_{(n- n_w ) + 1} = c_{(n- n_w) + 2} = \cdots = c_n$ i.e., $c \in \orbs_o$. \\
 Now, if $a_i + a_{i (\mod l ) + 1} = 1$, then we can see in Equation~\ref{eqn:cxk2}, the corresponding variable $ x_i $ will contribute to the sum. Let us denote $A_{n_1}=  (a_1, a_2, \ldots, a_{n_1}), A_{n_2}= (a_{n_1 + 1}, a_{n_1 + 2}, \ldots, a_{n_1 + n_2}), \cdots \text{ and } A_{n_w}= (a_{n -{n_w} + 1}, a_{n -{n_w} + 2}, \ldots, a_{n})$. Therefore from the Lemma~\ref{lem:BitXOR}, the number of $ x_i $'s are going to remain in Equation~ \ref{eqn:cxk2} for an $ c \in \FF_2^n \setminus \orbs_o$, is given as 
\begin{eqnarray}
   && \w(A_{n_1} ^ \wedge \overleftarrow{A_{n_1}}, A_{n_2} ^ \wedge \overleftarrow{A_{n_2}}, \ldots , A_{n_w} ^ \wedge \overleftarrow{A_{n_w}})   \nonumber \\
 =  && 2 ( \w(A_{n_1}) - |\{ (c_i, c_{i(\mod(n_1)) + 1}) : c_i + c_{i(\mod(n_1)) + 1} = 0 \text{ for } i \in [1,n_1] \}|)  \nonumber \\ 
+  &&  2 ( \w(A_{n_2}) - |\{ (c_i, c_{i(\mod(n_1 + n_2)) + 1}) : c_i + c_{i(\mod(n_1 + n_2)) + 1} = 0 \text{ for } i \in [n_1 +1 ,n_1 + n_2] \}|) \nonumber \\
+  &&  \cdots   \nonumber\\
+  &&  2 ( \w(A_{n_w}) - |\{ (c_i, c_{i(\mod(n_1 + n_2 + \ldots + n_w)) + 1}) : c_i + c_{i(\mod(n_1 + n_2 + \ldots + n_w) + 1} = 0 \text{ for } i \in [n-{n_w} + 1, n ] \}|) \nonumber \\
= && 2 \w(a) - 2 \times ( |\{ (c_i, c_{i(\mod(n_1)) + 1}) : c_i + c_{i(\mod(n_1)) + 1} = 0 \text{ for } i \in [1,n_1] \}|  \nonumber \\
+ && |\{ (c_i, c_{i(\mod(n_1 + n_2)) + 1}) : c_i + c_{i(\mod(n_1 + n_2)) + 1} = 0 \text{ for } i \in [n_1 +1 ,n_1 + n_2] \}|    \nonumber \\
+ && \cdots  \nonumber \\ 
+ && |\{ (c_i, c_{i(\mod(n_1 + n_2 + \ldots + n_w)) + 1}) : c_i + c_{i(\mod(n_1 + n_2 + \ldots + n_w) + 1} = 0 \text{ for } i \in [n-{n_w} + 1, n ] \}| ) \nonumber 
\end{eqnarray}
Since, $x \in \Ekn{k}{n}$, we will assign $i$ number of $1$'s to the variables that are present in the Equation~\ref{eqn:cxk2} such that $a. (x+ \psi(x))=1$ and  $(k-i)$ $1$'s are assign to the variables which are not present in the Equation~\ref{eqn:cxk2}. This implies,
$$\left| \{x \in \Ekn{k}{n} : c \cdot (x + \psi(x))= 1\} \right|
= \left \{ \begin{array}{ll}
\sum_{\substack{i=0 \\ i:odd}}^k {d \choose i} {{n-d} \choose {k-i}}  &  \text{ if } c \in \FF_2^n \setminus \orbs_o  \\
0              &  \text{ if } c \in \orbs_o.
\end{array} \right. $$
where $ d = \w(A_{n_1} ^ \wedge \overleftarrow{A_{n_1}}, A_{n_2} ^ \wedge \overleftarrow{A_{n_2}}, \ldots , A_{n_w} ^ \wedge \overleftarrow{A_{n_w}})$.
\end{proof}
...
Then $$\NLk{k}(f) ~\geq ~ 
\left\{ \begin{array}{ll}
       \frac{|\Ekn{k}{n}|}{2} - \frac{1}{2}\max_{a\in \FF_2^n}|\sum_{\substack{i=0 \\ i:odd}}^k {d \choose i} {(n-d) \choose (k-i)}| & \text{ if }  k \not\preceq n \\
        \frac{|\Ekn{k}{n}|}{2} - \frac{1}{2}\max_{a\in \FF_2^n}|\sum_{\substack{i=0 \\ i:odd}}^k {d \choose i} {(n-d) \choose (k-i)} + 1 | & \text{ if } k \preceq n
    \end{array}   \right.$$
\end{theorem}
\begin{proof}
Let $\psi \in \Sn$ be the permutation defined as in Equation~\ref{eqn:cycperm}. 
Let $\orbs_k$ be the set of all orbits of the group action $G$ on $\Ekn{k}{n}$.
...
The \textcolor{red}{weightwise} Walsh spectrum of $f_\psi$ at $a \in \Ekn{k}{n}$ is as follows. (\textcolor{red}{weightwise Walsh spectrum needs to be defined at $NL_k$})
...
\implies |\wtk{f}{k}(a)| & \leq & |\sum_{\orb \in \orbs_{e,k}} \sum_{x \in \orb} (-1)^{ f(x) + a \cdot x}| + 1
\end{eqnarray}
as the number of odd orbits is at most $1$ and its cardinality is $1$. Further,
...
\end{eqnarray}
The vectors $x$ for which $((-1)^{ a \cdot x} - (-1)^{a \cdot \psi(x)}) = 0$ i.e., $a \cdot (x + \psi(x))= 0$ have contribution $0$ to the sum in Equation~\ref{eqn:wse_k}. Hence, we have
\begin{eqnarray*}
\sum_{\orb \in \orbs_{e,k}} \sum_{x \in \orb} (-1)^{ f(x) + a \cdot x} 
& = & \frac{1}{2} \left[ \sum_{\orb \in \orbs_{e,k}} \sum_{x \in \orb : a \cdot (x + \psi(x))= 1} (-1)^{f(x)} ((-1)^{ a \cdot x} - (-1)^{a \cdot \psi(x)})  \right] \\
& = & \sum_{\orb \in \orbs_{e,k}} \sum_{x \in \orb : a \cdot (x + \psi(x))= 1 } (-1)^{f(x)+ a \cdot x} 
= \sum_{x \in \Ekn{k}{n} \setminus \{z\} : a \cdot (x + \psi(x))=1} (-1)^{f(x)+ a \cdot x}
\end{eqnarray*}
where $z$ (if any) is the vector of orbit cardninality odd.
Now, using the Lemma~\ref{lem:setSize_k}, we have an upper bound to the sum
\begin{eqnarray}
\left| \sum_{\orb \in \orbs_{e,k}} \sum_{x \in \orb} (-1)^{ f(x) + a \cdot x} \right| 
    & = & \left|\sum_{x \in \Ekn{k}{n} \setminus \orb_o : a \cdot (x + \psi(x))=1} (-1)^{f(x)+ a \cdot x}\right| \nonumber\\
    & \leq & \left\{
    \begin{array}{ll}
     \sum_{\substack{i=0 \\ i:odd}}^k {d \choose i} {(n-d) \choose (k-i)}   & \text{ if } a \in \FF_2^n \setminus \orbs_o \\
        0           & \text{ if } a \in \orbs_o. \label{eqn:sume_k}
    \end{array}   \right.  
\end{eqnarray} 
where $ d = \w(A_{n_1} ^ \wedge \overleftarrow{A_{n_1}}, A_{n_2} ^ \wedge \overleftarrow{A_{n_2}}, \ldots , A_{n_w} ^ \wedge \overleftarrow{A_{n_w}})$.
Hence, from Equation~\ref{eqn:ws_k}%, Equation~\ref{eqn:sumo_k} 
and Equation~\ref{eqn:sume_k}, we have the following bounds for $\wtk{f}{k}$ at $a \in \FF_2^n$ for $\Ekn{k}{n}$ depending on $ k \not\preceq n $ and $ k \preceq n$. \\
For $ k \not\preceq n $, we have 
...
|\wtk{f}{k}(a)| \leq \left\{ \begin{array}{ll}
        \sum_{\substack{i=0 \\ i:odd}}^k {d \choose i} {(n-d) \choose (k-i)} + 1 & \text{ if } a \in \FF_2^n \setminus \orbs_o \\
        1      & \text{ if } a \in \orbs_o
    \end{array}   \right. 
\end{equation}
Hence, the nonlinearity of $f_\psi$ satisfies
\begin{eqnarray*}
\NLk{k}(f_\psi) & ~\ge~ & 2^{n-1}-\frac{1}{2} \max_{a\in \FF_2^n}| \wtk{f}{k}(a)|
~\geq ~ 
\left\{ \begin{array}{ll}
       \frac{|\Ekn{k}{n}|}{2} - \frac{1}{2}\max_{a\in \FF_2^n}|\sum_{\substack{i=0 \\ i:odd}}^k {d \choose i} {(n-d) \choose (k-i)}| & \text{ if }  k \not\preceq n \\
        \frac{|\Ekn{k}{n}|}{2} - \frac{1}{2}\max_{a\in \FF_2^n}|\sum_{\substack{i=0 \\ i:odd}}^k {d \choose i} {(n-d) \choose (k-i)} + 1 | & \text{ if } k \preceq n
...
\begin{lemma}
Let $a = (a_1, a_2, \ldots, a_n) \in \FF_2^n$. Let $ \rho(a)$ be denoted as the cyclic left shift of $a$ by $ 1 $ bits and $ a + b$ be denoted the vector bitwise XOR operation. Then $$ \w(a + \rho(a) ) = 2 ( \w(a) - |\{ (a_i, a_{i (\mod(n)) + 1}) : a_i + a_{i (\mod(n) + 1} = 0 \text{ for } i \in [1,n] \}|). $$
\end{lemma}
\fi
...
\end{lemma} 
\begin{proof}
Let $x = (x_1,x_2, \ldots,x_n) \in \Ekn{n}{k}$ and $c = (c_1,c_2,\ldots,c_n) \in \FF_2^n$. Then as in Equation~\ref{eqn:cx2}, we have 
\begin{eqnarray*}
c \cdot (x+\psi(x)) & = & (c+\psi^{-1}(c)) \cdot x \label{eqn:cxk2}
\end{eqnarray*}
is a linear function on $n$ variable defined over the slice $\Ekn{n}{k}$.
%Consider $c+\psi^{-1}(c) = (\overline c_1, \overline c_2, \ldots, \overline c_n) \in \FF_2^n$ where $\overline c_1 = c_1 + c_2, \overline c_2 = c_2 + c_3, \ldots, \overline c_n = c_n + c_{n-n_w+1}$.
...
= \frac{1}{2} (|\Ekn{n}{k}| - \kraw{k}{l}{n}).$$
\iffalse
If $ c \in \FF_2^n \setminus \orbs_o$, then we have
\begin{align*}
\w(c \cdot (x + \psi(x)) & = \sum_{\substack{j=0 \\ odd}}^k {\w(c+\psi(c)) \choose j}{ n - \w(c+\psi(c)) \choose  k-j}\\
                         & = \frac{1}{2} \left[{n \choose k} - \left(\sum_{j=0}^k {\w(c+\psi(c)) \choose j}{ n - \w(c+\psi(c)) \choose  k-j} - 2 \sum_{\substack{j=0 \\ odd}}^k {\w(c+\psi(c)) \choose j}{ n - \w(c+\psi(c)) \choose  k-j} \right) \right] \\
                         & = \frac{1}{2} \left[{n \choose k} - \sum_{\substack{j=0 \\ odd}}^k (-1)^{j} {\w(c+\psi(c)) \choose j}{ n - \w( c +\psi(c) \choose  k-j}   \right]     \\
                         & = \frac{1}{2} \left[ |\Ekn{k}{n}| - \kraw{k}{\w(c + \psi(c))}{n} \right]                        
\end{align*}
Hence,
$$\left| \{x \in \Ekn{k}{n} : c \cdot (x + \psi(x))= 1\} \right|
= \left \{ \begin{array}{ll}
\frac{1}{2} \left[ |\Ekn{k}{n}| - \kraw{k}{\w(c + \psi(c))}{n} \right]  &  \text{ if } c \in \FF_2^n \setminus \orbs_o  \\
0              &  \text{ if } c \in \orbs_o.
\end{array} \right. $$ \fi
\end{proof}
...
\iffalse
\left\{ \begin{array}{ll}
       \displaystyle{\frac{|\Ekn{n}{k}|}{2} - \frac{1}{4}\max_{a\in \FF_2^n}|\Ekn{n}{k}| - \kraw{k}{l}{n}} & \text{ if }  k \not\preceq n \\
    \displaystyle{\frac{|\Ekn{n}{k}|}{2} - \frac{1}{4}\max_{a\in \FF_2^n} |\Ekn{n}{k}| - \kraw{k}{l}{n}) - \frac{1}{2}} & \text{ if } k \preceq n
    \end{array}   \right.$$
where $l = \w(c+\psi^{-1}(c))$.\fi
\end{theorem}
\begin{proof}
Let $\orbs_k$ be the set of all orbits of the group action $G = \langle \psi \rangle$ on $\Ekn{k}{n}$.
Let $\orbs_{e,k}$ be the set of all orbits in $\orbs_k$ of cardinality even and $\orbs_{o,k}$ be the set of orbits in $\orbs_{k,n}$ of cardinality odd of $\orbs_k$. \\
The restricted Walsh spectrum of $f_\psi$ at $a \in \FF_2^n$ is as follows.
\begin{eqnarray} \label{eqn:ws_k}
\wtk{f}{k}(a) & =  & \sum_{x \in \Ekn{k}{n}} (-1)^{f(x)+a \cdot x} 
= \sum_{\orb \in \orbs_k} \sum_{x \in \orb} (-1)^{f(x)+ a \cdot x} \nonumber \\ 
& = &\sum_{\orb \in \orbs_{e,k}} \sum_{x \in \orb} (-1)^{ f(x) + a \cdot x} + \sum_{\orb \in \orbs_{o,k}} \sum_{x \in \orb} (-1)^{ f(x) + a \cdot x} \nonumber \\
...
\begin{equation}\label{eqn:ws2_even_k}
|\wtk{f}{k}(a)| \leq \left\{ \begin{array}{ll}
        \frac{1}{2} \left[ |\Ekn{k}{n}| - \kraw{k}{\w(a + \psi(a))}{n} \right] & \text{ if } a \in \FF_2^n \setminus \orbs_o \\
        0        & \text{ if } a \in \orbs_o
    \end{array}   \right. 
\end{equation}
and for $ k \preceq n$, we have 
\begin{equation}\label{eqn:ws2_odd_k}
|\wtk{f}{k}(a)| \leq \left\{ \begin{array}{ll}
        \frac{1}{2} \left[ |\Ekn{k}{n}| - \kraw{k}{\w(a + \psi(a))}{n} \right] + 1 & \text{ if } a \in \FF_2^n \setminus \orbs_o \\
        1      & \text{ if } a \in \orbs_o
    \end{array}   \right. 
\end{equation}\fi
Hence, the nonlinearity of $f_\psi$ satisfies
\begin{eqnarray*}
\NLk{k}(f_\psi) ~ = ~ \frac{|\Ekn{n}{k}|}{2} -\frac{1}{2} \max_{a\in \FF_2^n}| \wtk{f}{k}(a)|
...
\end{array}   \right.
\end{eqnarray*}
%\[\NLk{k}(f) = \frac{|\Ekn{n}{k}|}{2}- \frac{1}{2}\max_{a \in \FF_2^n} |\wtk{f}{k}(a)|
\end{proof}
...
\end{theorem}
\begin{proof}
Let $x = (y,z) \in \FF_2^n$, where $y\in \FF_2^{n-n_w}$ and $z \in \FF_2^{n_w}$. Let denote 
...
Let $\orbs$ be the set of all orbits of $G = \langle \rho_{n_w} \rangle$ acts on $\FF_2^{n_w}$.
For the simplicity, we denote $f = f_\psi$.
Then, the Walsh-Hadamard transform of $f$ at $a = (b,c) \in \FF_2^n$ with $b \in \FF_2^{n-n_w}$ and $c \in \FF_2^{n_w}$ is
...
%\end{align*}	
%$$ \wt{f_\psi}(a) = \sum_{x\in \FF_2^n} (-1)^{f_\psi(x)+a \cdot x} 
...
\begin{eqnarray*}
\wt{f}(a) & = & \sum_{x\in \FF_2^n} (-1)^{f(x)+a \cdot x}
= \sum_{z \in \FF_2^{n_w}} \sum_{y \in \FF_2^{n-n_w}}(-1)^{f(y,z)+b \cdot y+c \cdot z} \\
& = & \sum_{\orb \in \orbs} \sum_{z \in \orb} \sum_{y \in \FF_2^{n-n_w}}(-1)^{fs(y,z)+b \cdot y+c \cdot z}
\end{eqnarray*}
Here, $\orb_0 = \{0^{n_w}\}$ and $\orb_1 = \{1^{n_w}\}$ are the only orbits of cardinality $1$ in $\FF_2^{n_w}$. 
Denote $\hat{\orbs} = \orbs \setminus \{\orb_0,\orb_1\}$ which is the set of all orbits of cardinality even. Hence,
\begin{equation}\label{eq:wt}
\wt{f}(a) = \sum_{\orb \in \hat{\orbs}} \sum_{z \in \orb} \sum_{y \in \FF_2^{n-n_w}} (-1)^{f(y,z) + b\cdot y + c\cdot z} + \sum_{z \in \{0^{n_w}, 1^{n_w}\}} \sum_{y \in \FF_2^{n-n_w}}(-1)^{f(y,z) + b\cdot y + c \cdot z}
\end{equation}	
...
\begin{align*}
|\sum_{z \in \{0^{n_w}, 1^{n_w}\}} \sum_{y \in \FF_2^{n-n_w}}(-1)^{f(y,z) + b\cdot y + c \cdot z}|
 %& = |\sum_{y \in \FF_2^{n-n_w}} (-1)^{f(y,0^{n_w})+b \cdot y} + \sum_{y \in \FF_2^{n-n_w}} (-1)^{f(y,1^{n_w})+b \cdot y + c \cdot 1^{n_w}}|  \\
      & \leq |\sum_{y \in \FF_2^{n-n_w}} (-1)^{f(y,0^{n_w})+b \cdot y}| + |\sum_{y \in \FF_2^{n-n_w}} (-1)^{f(y,1^{n_w})+b \cdot y + c \cdot 1^{n_w}}|  \leq   2^{n-n_w+1}.
\end{align*}
Then, we have from the first sum of Equation~\ref{eq:wt} as 
\begin{align*}
\sum_{\orb \in \hat{\orbs}} \sum_{z \in \orb} \sum_{y \in \FF_2^{n-n_w}} (-1)^{f(y,z) + b\cdot y + c\cdot z}
& = \frac{1}{2} \sum_{\orb \in \hat{\orbs}} \sum_{z \in \orb} \sum_{y \in \FF_2^{n-n_w}} \left((-1)^{f(y,z) + b\cdot y + c\cdot z} + (-1)^{f(y,\pi_w(z)) + b\cdot y + c\cdot \pi_w(z)} \right)\\
...
& =  \frac{1}{2} \sum_{\orb \in \hat{\orbs}} \sum_{z \in \orb} \sum_{y \in \FF_2^{n-n_w}}
...
\end{proposition}
\begin{proof}
	Let $ c = 0^n \in \FF_2^n$. Then $c(z + \pi(z))=0$. Hence $|\{ z \in \FF_2^{n} \setminus \{0_{n},1_{n}\}, c(z+\pi(z))=1   \}| = 0$. Similarly, if $c = 1^n$, then  $$|\{ z \in \FF_2^{n} \setminus \{0_{n},1_{n}\}, 1^n(z+\pi(z))=1   \}| = |\{ z \in \FF_2^{n} \setminus \{0_{n},1_{n}\}, (z_1 + z_n)+(z_2 + z_1)+ \cdots (z_n + z_{n-1})= 1 \}|$$ is given as follows:
	\begin{align*}
	(z_1 + z_n)+(z_2 + z_1)+ \cdots (z_n + z_{n-1}) = 2(z_1 + z_2 + \cdots + z_n) 
	\end{align*}
	This implies, there is no $z \in \FF_2^n$ such that the above equation will be 1. \\
Let $c=(c_1,c_2,\ldots,c_n) \in \FF_2^n \setminus \{0^n,1^n\}$. Then 
\begin{align*}
         & c_1(x_1 + x_n) + c_2(x_2 + x_1) + \ldots + c_n (x_n + x_{n-1})  = 0 \\
\implies &  x_1 ( c_1 + c_2) + x_2 (c_2 + c_3) + \ldots + x_{n-1}(c_{n-1} + c_n) + x_n(c_n + c_1)= 0  
\end{align*}
Hence, this can be written as $A= \{ x \in \FF_2^n : u. x = 0\}$ for $ u = (u_1,u_2, \ldots,u_n) \in \FF_2^n$ where $ u_i = c_i + c_{i+1}$ for $  1 \leq i \leq {n-1} $, and $u_n = c_n + c_1$. Since, $ u \neq 0 $ as $c \not\in \{0^n,1^n\}$ and $A$ is a hyperplane, implies that the cardinality of the set $A$ is $2^{n-1}$.
\end{proof}
\fi
